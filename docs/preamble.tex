...
% Единичен ред, откъснат от абзаца си в началото или в края на страница, е дефект
% на пренасянето, не на текста. Наказанията го забраняват структурно, вместо
% абзацът да бъде преписван заради подредбата на страницата.
\widowpenalty=10000
\clubpenalty=10000
...
% Инлайн код/идентификатори. Имената на тестовете в този проект стигат шестдесет
% знака и нямат къде да се прекъснат: \texttt не пренася, а правилото „без
% пренасяния“ (А.4) забранява и това. Затова вътре в \code долната черта носи
% \allowbreak, тоест разрешено, но неотбелязано място за прекъсване. Това е
% типографска поправка, а не съкращаване на идентификатора.
\DeclareRobustCommand{\code}[1]{{\ttfamily\renewcommand{\_}{\textunderscore\allowbreak}#1}}
% Глифи за таблиците с присъди.
\newcommand{\cmark}{\ensuremath{\checkmark}}
\newcommand{\xmark}{\ensuremath{\times}}

% Редът по-долу е закоментиран. Той зареждаше генерирани макроси от друг проект и
% без него компилацията спира с „File not found“. Когато tools/ на този проект
% започне да генерира числата от измерванията, редът се разкоментира и се насочва
% натам.
% \input{generated/measurement_macros}

% Изображение с тънка черна рамка. Рамката НЕ е украса: заснетите екрани на
% инструменти за анализ са със светъл фон и се сливат с бялата страница, така че
% без нея не се вижда къде свършва изображението и започва листът.
...
\lstset{
    ...
    % Идентификатори на този проект. Промяната е чисто козметична, засяга само
    % оцветяването на листингите.
    emph={sentinel, handshake, client_hello, server_hello, cipher_suite, extension,
          cert_chain, path_builder, trust_store, revocation, ocsp, crl,
          admission, syn_cookie, rate_limiter, conn_table, capture, packet},
